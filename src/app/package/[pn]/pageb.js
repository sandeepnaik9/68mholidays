...
import CalerndarI from '../../../../public/assets/icon-calendar-color.svg'
...
const Package = ({ params }) => {
  ...
  const [tourIncludes,setTourIncludes] = useState({"hotel":true,"meals":true,"transport":true,"sightseeing":true})
  ...
  useLayoutEffect( () => {
    ...
    async function  getPacka (){
      ...
      if(packageName==""){
      setTitle(res.data[0].package_name)
      setPackageName(res.data[0].package_name)
      setFeaturedImage(res.data[0].BannerImage)}
    }

    getPacka();
  }, [isEditingS,packageName])
  ...
  return (
    ...
    <div>
      <div className='container pt-5'>
        <div className="row w-100">
          <div className="col-8 d-flex flex-column justify-content-center">
           {/* {isEditingS?(<input className='fw-semibold h3' onChange={(e)=>{setPackageName(e.target.value);setTitle(e.target.value)}} defaultValue={title} style={{outline:'none',border:"none",background:"none"}} />):<h1 className='fw-semibold h3'>{title}</h1>}  */}
           <h1 className='fw-semibold h3'>{title}</h1>
          ...
          <div className="col-4 row d-flex justify-content-end ">
            <div className='col-7'>
              ...
              <div className='d-flex justify-content-end align-items-center'>
                <span className='h-100 me-2' style={{ fontWeight: "400", fontSize: "12px" }}>Starts From</span><h4 className='h4 d-flex p-0 m-0 h-100 justify-content-end align-items-center'>₹44,000</h4>
              </div>
            ...
            <div className='d-flex col-5  flex-column justify-content-center align-items-center'>
              <div className='w-100 d-flex justify-content-center align-items-center' style={{ height: "45px", backgroundColor: "rgb(255, 216, 0)", appearance: "button", borderRadius: "5px", fontFamily: "Muli, sans-serif", fontSize: "14px" }}>
                Dates & Prices
              </div>
              <div className='w-100 d-flex justify-content-center align-items-center' style={{ height: "25px!important", width: "150px!important", fontWeight: "400", margin: "10px 0 10px 0", backgroundColor: "rgb(202, 206, 220)", appearance: "button", borderRadius: "5px", fontFamily: "Muli, sans-serif", fontSize: "10px" }}>
                View Pricing Table
              </div>
            </div>
          </div>
        ...
        <div className='row'>
          ...
          <div className='col-4 '>
            <div style={{ backgroundColor: "rgb(245, 245, 245)", padding: "22px", borderRadius: "10px" }}>
      ...
      {isAuthS&&(<div className='d-flex editIcon justify-content-center rounded-circle text-white align-items-center position-absolute' onClick={()=>{dispatch(setIsEditing(!isEditingS));console.log(isEditingS)}} style={{right:100,bottom:50,cursor:"pointer",background:"#0582BE",width:"50px",height:"50px"}}>{isEditing?(<i className="fas fa-save"></i>):(<i className="fas fa-edit"></i>)}</div>)}
      </div>
    </>
  )
}
...
const getPackage = async () =>{
  const packageD = await (await fetch('/api/package/getPackage',{method:"GET"})).json()
  return packageD
}